...
context("Filter products", () => {
    ...
    describe("Filter products in catalogue", () => {
      ...
      it("should sort by name", () => {
        ...
        let productNames = []
        cy.get(CATALOGUE.MAIN_PAGE.PRODUCT_CONTAINERS).find('h3').each($productName => {
          productNames.push($productName.text())
          productNames.sort()
        })
        //Compare sorted list with actual order
        cy.get(CATALOGUE.MAIN_PAGE.PRODUCT_CONTAINERS).find('h3').each(($productName, index) => {
          expect($productName.text()).to.equal(productNames[index])
        })
      })

      it("should set products number per page", () => {
        cy.get(CATALOGUE.MAIN_PAGE.TOTAL_PRODUCTS_TEXT).as('productsText').invoke('text').then(totalText => {
          ...
          cy.get(CATALOGUE.MAIN_PAGE.SHOW_PRODUCTS_BUTTONS).as('buttons').then($buttons => {
            for (let i = 0; i < $buttons.length; i++) {
              cy.get('@buttons').eq(i).then($button => {
                const paginatedProductsCount = $button.text()
                cy.wrap($button).click()
                waitForCataloguePage()
                //Confirm that number of products is updated accordingly
                cy.get(CATALOGUE.MAIN_PAGE.PRODUCT_CONTAINERS).should('have.length', paginatedProductsCount)
                cy.get('@productsText').should('contain.text', `${paginatedProductsCount} of ${productsCount}`)
              })
            }
          })
        })
      ...
      it("should filter by products", () => {
        cy.get(CATALOGUE.MAIN_PAGE.TOTAL_PRODUCTS_TEXT).as('productsText').invoke('text').then(originalTotalText => {
          ...
          cy.get('@productsText').invoke('text').then(filteredTotalText => {
            expect(originalProductsCount).to.not.equal(getLastNumberInString(filteredTotalText))
          })
          //Clear filtering & confirm the old products count is back
          clearFilter()
          cy.get('@productsText').invoke('text').then(clearedTotalText => {
            expect(originalProductsCount).to.equal(getLastNumberInString(clearedTotalText))
          })
        })
      })
      ...
      it("should catalogue dropdown options be included in filer sidebar", () => {
        let filterOptions = []
        cy.get(CATALOGUE.FILTERS.SIDEBAR).find('.checkbox').each($checkbox => {
