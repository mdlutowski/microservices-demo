...
context("Register a new user", () => {
    ...
    describe("Scenario - Successful registration as a new user", () => {
      it("should open Register modal", () => {
        openRegisterModal()
        cy.get(HOME.REGISTER_MODAL.MODAL).find(HOME.REGISTER_MODAL.HEADER).should('have.text', TRANSLATION.REGISTER.HEADER)
        cy.get(HOME.REGISTER_MODAL.USERNAME).invoke('attr', 'placeholder').should('equal', TRANSLATION.REGISTER.USERNAME)
        cy.get(HOME.REGISTER_MODAL.FIRST_NAME).invoke('attr', 'placeholder').should('equal', TRANSLATION.REGISTER.FIRST_NAME)
        cy.get(HOME.REGISTER_MODAL.LAST_NAME).invoke('attr', 'placeholder').should('equal', TRANSLATION.REGISTER.LAST_NAME)
        cy.get(HOME.REGISTER_MODAL.EMAIL).invoke('attr', 'placeholder').should('equal', TRANSLATION.REGISTER.EMAIL)
        cy.get(HOME.REGISTER_MODAL.PASSWORD).invoke('attr', 'placeholder').should('not.exist')
        cy.get(HOME.REGISTER_MODAL.MODAL).find(HOME.REGISTER_MODAL.REGISTER_BUTTON).should('contain.text', TRANSLATION.REGISTER.REGISTER_BUTTON)
      })
      ...
      it("should register a new user", () => {
        ...
        cy.get(HOME.REGISTER_MODAL.MESSAGE).should('have.text', TRANSLATION.REGISTER.SUCCESSFUL_MESSAGE)
        //Confirm user is logged in & account menu is available & 'log in' link is no longer visible
        cy.get(HOME.MAIN_PAGE.LOGGEDIN_LINK).should('contain.text', `${TRANSLATION.LOGIN.LOGGED_IN} ${firstName} ${lastName}`)
