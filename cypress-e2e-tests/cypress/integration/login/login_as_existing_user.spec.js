...
context("Log in an existing user", () => {
  ...
  describe("Scenario - Successful login using top container", () => {
    it("should open Login modal", () => {
      openLoginModal();
      cy.get(HOME.LOGIN_MODAL.MODAL)
        .find(HOME.LOGIN_MODAL.HEADER)
        .should("have.text", TRANSLATION.LOGIN.HEADER);
      cy.get(HOME.LOGIN_MODAL.USERNAME)
        .invoke("attr", "placeholder")
        .should("equal", TRANSLATION.LOGIN.USERNAME);
      cy.get(HOME.LOGIN_MODAL.PASSWORD)
        .invoke("attr", "placeholder")
        .should("equal", TRANSLATION.LOGIN.PASSWORD);
      cy.get(HOME.LOGIN_MODAL.MODAL)
        .find(HOME.LOGIN_MODAL.LOGIN_BUTTON)
        .should("contain.text", TRANSLATION.LOGIN.LOGIN_BUTTON);
    });

    it("should log in", () => {
      login(mainUser, mainPassword);
      //Confirm successful message
      cy.get(HOME.LOGIN_MODAL.MESSAGE).should(
        "have.text",
        TRANSLATION.LOGIN.SUCCESSFUL_MESSAGE
      );
      ...
      cy.get(HOME.MAIN_PAGE.LOGGEDIN_LINK).should(($text) => {
        expect($text.text().toLowerCase()).to.contain(
          `${TRANSLATION.LOGIN.LOGGED_IN} ${mainUser}`.toLowerCase()
        );
      });
    ...
    it("should log out and log another user in", () => {
      cy.fixture("/testdata/users").then((users) => {
        ...
        cy.get(HOME.MAIN_PAGE.LOGGEDIN_LINK).should(($text) => {
          expect($text.text().toLowerCase()).to.contain(
            `${TRANSLATION.LOGIN.LOGGED_IN} ${secondUsername}`.toLowerCase()
          );
        });
        cy.get(HOME.MAIN_PAGE.ACCOUNT_TAB).should("be.visible");
      });
  ...
  describe("Scenario - Unsuccessful login using top container", () => {
    ...
    it("should not log in if user does not exist", () => {
      cy.fixture("/testdata/users").then((users) => {
        ...
        cy.get(HOME.LOGIN_MODAL.MESSAGE).should(
          "have.text",
          TRANSLATION.LOGIN.INVALID_LOGIN
        );
    ...
    it("should not log in without credentials", () => {
      ...
      //clode modal
      cy.get(HOME.LOGIN_MODAL.MODAL)
        .find(HOME.LOGIN_MODAL.CLOSE_BUTTON)
        .click();
    });
  });
});
