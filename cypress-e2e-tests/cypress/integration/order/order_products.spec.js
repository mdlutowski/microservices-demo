  ...
  beforeEach(() => {
    ...
    describe("Order a product from catalogue", () => {
      it("should order a product", () => {
        ...
        cy.get(HOME.MAIN_PAGE.BASKET_BUTTON).find('span').should('have.text', `1 ${TRANSLATION.BASKET.BASKET_TEXT}`)
          ...
          cy.get(BASKET.ORDER_SUBTOTAL).then($orderSubtotal => {
            ...
            cy.get(BASKET.ORDER_SHIPPING).then($orderShipping => {
              ...
              cy.get(BASKET.ORDER_TOTAL).then($orderTotal => {
                ...
                expect(orderTotal).to.eq(parseAmount(orderSubtotal + orderShipping))
                //Finalize order
                completeOrder()
                //Confirm order is visible on customer account
                cy.get(ORDERS.TABLE_ORDERS).find('tr').last().find('th,td').should($cells => {
                  expect($cells.eq(0).text()).to.match(/^# \w*$/)
                  expect($cells.eq(1).text()).to.contain(dayjs().format('YYYY-MM-DD'))
                  expect($cells.eq(2).text()).to.equal(`$ ${orderTotal}`)
                  expect($cells.eq(3).text().trim()).to.equal(TRANSLATION.ORDERS.SHIPPED)
                  expect($cells.eq(4).text().trim()).to.equal(TRANSLATION.ORDERS.VIEW)
                })
              })
            })
          })
      ...
      it("should order multiple product", () => {
        ...
        cy.get(HOME.MAIN_PAGE.BASKET_BUTTON).find('span').should('have.text', `2 ${TRANSLATION.BASKET.BASKET_TEXT}`)
    ...
    describe("Fail to order product from catalogue", () => {
      it("should decline payment exceeding $100", () => {
        ...
        goToBasket()
        completeOrder(false)
        cy.get(BASKET.NOTIFICATION).should('contain.text', TRANSLATION.BASKET.TOO_HIGH_AMOUNT)
      })
      ...
      it("should fail to finalize order if shipping or payment information is missing", () => {
        ...
        goToBasket()
        completeOrder(false)
        cy.get(BASKET.NOTIFICATION).should('contain.text', TRANSLATION.BASKET.MISSING_INFORMATION)
      })
    })
})
